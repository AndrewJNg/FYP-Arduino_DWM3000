...
robot_start_positions = [-0.4 -0.4; 0.4 -0.4; 0 0; -0.4 0.4]
...
%     actual_positions = move_robot_step(actual_positions, 4, 1, 1);
    actual_positions = move_robot_step(actual_positions, 3, target_x, target_y);
...
function actual_positions = move_robot_step(actual_positions, robot_id, target_x, target_y)
    ...
    angle_noise_deg = -10 + (20)*rand();  % Random number between -10 and +10
    angle_noise_rad = deg2rad(angle_noise_deg);

    dx = 1/5*total_distance*cos(angle+angle_noise_rad);
    dy = 1/5*total_distance*sin(angle+angle_noise_rad);
