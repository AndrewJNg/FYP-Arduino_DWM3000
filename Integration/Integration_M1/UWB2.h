...
public:
  ...
  double processIncoming(uint16_t sender_id, int8_t pos[3], int8_t vel[3]) {
    setAnchorMode(true);
    return Anchor_waiting_for_response(sender_id, pos, vel);
  }
  ...
  double Anchor_process_received_message(uint16_t sender_id, int8_t pos[3], int8_t vel[3]) {
    ...
    if (frame_len <= sizeof(rx_buffer)) {
      ...
      if (memcmp(rx_buffer, const_receive_msg, ALL_MSG_COMMON_LEN + 2) == 0) {
        if (rx_buffer[9] == 0xE0) {
        ...
        } else if (rx_buffer[9] == 0xE2) {
          uint32_t T_poll_start, T_poll_end, T_reply1, T_round1, T_reply2;
          float clockOffsetRatio;
          double tof;
          ...
          resp_msg_get_ts(&rx_buffer[RESP_MSG_RESP_TX_TS_IDX + 4], &T_reply2);

          uint32_t T_round2 = T_poll_end - T_poll_start;
