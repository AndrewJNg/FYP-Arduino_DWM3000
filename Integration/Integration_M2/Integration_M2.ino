...
void AnchorTask(void* parameter) {
  ...
  while (true) {
    ...
    if (now - last_switch >= 100) {
      ...
      if (swap % 3 == 0) {
        distance_received = dwm3000.getDistance(0xAA, positions, velocities);  //takes around 5ms to obtain information for TWR
        ...
        if (distance_received != -1 && distance_received > 0) {
          ...
          dwm3000.getRobotInfo(dw_pos_a, dw_velocity_a);
        }

      } else if (swap % 3 == 1) {
        distance_received = dwm3000.getDistance(0xDD, positions, velocities);
        Serial.print("DIST D: ");
        ...
        if (distance_received != -1 && distance_received > 0) {
          ...
          Serial.print(dist_2_B);
          dwm3000.getRobotInfo(dw_pos_b, dw_velocity_b);
        }

      } else {
        distance_received = dwm3000.getDistance(0xEE, positions, velocities);
        Serial.print("DIST E: ");
        Serial.print(distance_received);
        ...
        if (distance_received != -1 && distance_received > 0) {
          ...
          dwm3000.getRobotInfo(dw_pos_c, dw_velocity_c);
        }
      }
      Serial.println();
      ...
      Serial.print(dwm3000.processIncoming(positions, velocities));
      Serial.print("\t Address: ");
      Serial.print(dwm3000.getRobotInfo(dw_pos_b, dw_velocity_b), HEX);
      Serial.println("");
    }
